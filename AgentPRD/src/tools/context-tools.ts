...
export const createContextTools = (
  ...
  set_work_context: tool({
    ...
    parameters: z.object({
      ...
      description: z
        .string()
        .describe("Detailed description of what we're working on"),
      goals: z
        .array(z.string())
        .default([])
        .describe('List of specific goals or tasks'),
      tags: z
        .array(z.string())
        .default([])
    ...
    execute: async ({ title, description, goals, tags }) => {
      return await contextManager.setWorkContext(
        title,
        description,
        goals,
        tags,
  ...
  get_work_context: tool({
    ...
    parameters: z.object({
      contextId: z
        .string()
        .default('')
        .describe('Optional specific context ID to retrieve'),
    }),
    execute: async ({ contextId }) => {
      return await contextManager.getWorkContext(
        contextId || undefined,
        userId
      );
    },
  ...
  list_work_contexts: tool({
    ...
    parameters: z.object({
      status: z
        ...
        .default('all')
        .describe('Filter contexts by status'),
      limit: z
        .number()
        .default(10)
  ...
  store_prd: tool({
    ...
    execute: async ({ title, content, contextId, status }) => {
      return await contextManager.storePRD(
        title,
        content,
        contextId || undefined,
        status,
  ...
  get_prd: tool({
    ...
    parameters: z.object({
      ...
      title: z
        .string()
        .default('')
        .describe('Search by PRD title (alternative to ID)'),
    }),
    execute: async ({ prdId, title }) => {
      return await contextManager.getPRD(
        prdId || undefined,
        title || undefined
      );
    },
  ...
  list_prds: tool({
    ...
    parameters: z.object({
      ...
      status: z
        .enum(['draft', 'review', 'approved', 'all'])
        .default('all')
    ...
    execute: async ({ contextId, status, limit }) => {
      return await contextManager.listPRDs(
        contextId || undefined,
        status,
        limit
      );
...
export class ContextManager {
  ...
  async getWorkContext(
    ...
    if (!targetContextId && userId) {
      const activeContextData = await this.ctx.kv.get(
        KV_NAMESPACE,
        `user:${userId}:active_context`
      );
